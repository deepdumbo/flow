...
lvalhand = uicontrol('Style', 'edit','Position', Lval_Pos,'String',sprintf('%6.3f',LevV), 'BackgroundColor', [1 1 1], 'FontSize', LVFntSz,'Callback', @WinLevChanged);
wvalhand = uicontrol('Style', 'edit','Position', Wval_Pos,'String',sprintf('%6.3f',Win), 'BackgroundColor', [1 1 1], 'FontSize', WVFntSz,'Callback', @WinLevChanged);
...
% -=< Window and level mouse adjustment >=-
    function WinLevAdj(varargin)
        ...
        [Rmin, Rmax] = WL2R(Win,LevV);
        caxis([Rmin, Rmax])
        set(lvalhand, 'String', sprintf('%6.3f',LevV));
        set(wvalhand, 'String', sprintf('%6.3f',Win));
        InitialCoord = get(0,'PointerLocation');
    end
